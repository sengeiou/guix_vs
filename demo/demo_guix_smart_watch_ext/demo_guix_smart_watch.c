...
static const GX_CHAR *lauguage_ch_day_names[7] = {
    "周末",
    "周一",
    "周二",
    "周三",
...
static struct date_string lauguage_string_table[MAIN_SCREEN_LANGUAGE_TABLE_SIZE];
...
static VOID* memory_allocate(ULONG size)
{
    return res_alloc((size_t)size);
}

static VOID memory_free(VOID* mem)
{
    res_free(mem);
}
...
static int guix_main(UINT(*drv_setup)(GX_DISPLAY*))
{
    ...
    lauguage_string_table[LANGUAGE_ENGLISH] = lauguage_en_string;
    lauguage_string_table[LANGUAGE_CHINESE] = lauguage_ch_string;
...
VOID clock_update()
{
    ...
    sprintf(date_string_buffer, "%s, %s %02d", 
        dstring->day[0],
        dstring->mon[local_time.wMonth - 1], 
        local_time.wDay);
